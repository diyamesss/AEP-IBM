...
const PORTFOLIO_NAME = 'Customer'
const SHOWCASE = PORTFOLIO_SHOWCASE['customer']
...
const MAIN_CHALLENGES = [
  'Difficulties in finding resources with Power Builder skills.',
]

const MITIGATIONS = [
  'Pursuing options to broaden ability to deliver and support from different Geos.',
]
...
function ChartCardPanel({ card }: { card: ChartCard }) {
  ...
  return (
    <article
      id={card.id}
      className={[styles.chartCard, card.fullWidth ? styles.chartCardFullWidth : ''].filter(Boolean).join(' ')}
          ...
          <div className={styles.highlightsGrid}>
            <div className={styles.hlCardRed}>
              <div className={styles.hlCardContent}>
                ...
                {MAIN_CHALLENGES.map((text, i) => <p key={i} className={styles.hlCardText}>{text}</p>)}
              </div>
            ...
            <div className={styles.hlCardRed}>
              <div className={styles.hlCardContent}>
                ...
                {MITIGATIONS.map((text, i) => <p key={i} className={styles.hlCardText}>{text}</p>)}
              </div>
            </div>
          </div>
        </div>
